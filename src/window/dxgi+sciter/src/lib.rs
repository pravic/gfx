...
pub type SciterHost = Rc<sciter::Host>;


lazy_static! {
  static ref _API: &'static sciter::ISciterAPI = { sciter::SciterAPI() };
}
...
impl Window {
    pub fn get_host(&self) -> SciterHost {
      self.host.clone()
    }
    ...
    for dt in driver_types.iter() {
        match gfx_device_dx11::create(*dt, &swap_desc, color_format) {
            Ok((device, factory, chain, color)) => {
                ...
                let schost = window::init_sciter(hwnd, chain).expect("Could not create Sciter on DX Window.");
                let win = Window {
                    hwnd: hwnd,
                    swap_chain: chain,
                    host: Rc::new(schost),
